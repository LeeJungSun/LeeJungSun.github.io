...
gulp.task('html-deploy', ['clean-html-folders'], function () {
    return gulp.src([paths.html, '!src/html/include/*.html'])
        .pipe(fileinclude({
            prefix: '@@',
            basepath: '@file'
        }))
...
gulp.task('css-concat', function () {
    return gulp.src([bases.dest + 'css/styles/**/*.css', '!css/styles/**/*.min.css'])
        .pipe(plumber(plumberOption))
...
gulp.task('minify-css', ['css-concat'], function () {
    gulp.src([bases.dest + 'css/styles/styles.css'])
        .pipe(plumber(plumberOption))
        .pipe(cssmin())
        .pipe(rename({
            suffix: '.min'
...
gulp.task('css-libs-concat', function () {
    return gulp.src([bases.dest + 'css/libs/**/*.css', '!css/libs/**/*.min.css'])
        .pipe(plumber(plumberOption))
...
gulp.task('generate-css', function () {
    runSequence('clean-css-folders', 'css-deploy', 'css-libs-deploy', 'minify-libs-css', 'minify-css');
});
